...
export default function Cards({ stays }) {
  return (
    <div className="cardsContainer grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 p-6">
      {stays.map((hotel, index) => (
        <div
          key={index}
          className="overflow-hidden transition-transform duration-300 hover:scale-105 flex flex-col"
        >
          <img
          ...
          <div className="flex items-center justify-between p-2">
            {hotel.superHost && (
              <p className="border border-gray-800 rounded-full px-2 py-1 text-xs md:text-md font-bold text-gray-800">
                Super Host
              </p>
            )}
